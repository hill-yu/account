...
DAILY_DIMENSION_REQUIRED_CSV_COLUMNS = (
    "Dimension.SITE_NAME", "Dimension.COUNTRY_CODE", "Dimension.AD_UNIT_ID", "Dimension.AD_UNIT_NAME",
    "Column.AD_EXCHANGE_RESPONSES_SERVED", "Column.AD_EXCHANGE_TOTAL_REQUESTS",
    "Column.AD_EXCHANGE_LINE_ITEM_LEVEL_IMPRESSIONS", "Column.AD_EXCHANGE_LINE_ITEM_LEVEL_CLICKS",
    "Column.AD_EXCHANGE_LINE_ITEM_LEVEL_REVENUE", "Column.AD_EXCHANGE_LINE_ITEM_LEVEL_AVERAGE_ECPM",
)
...
def parse_daily_dimension_report_csv(raw_csv: str, *, report_date: date) -> list[dict[str, object]]:
    ...
    for row in reader:
        ...
        rows.append({
            "report_date": report_date.isoformat(), "url_id": _site_name_or_placeholder(row), "url": _site_name_or_placeholder(row),
            "ad_country_code": (row.get("Dimension.COUNTRY_CODE") or "UNKNOWN").strip() or "UNKNOWN",
            "ad_country_name": (row.get("Dimension.COUNTRY_CODE") or "UNKNOWN").strip() or "UNKNOWN",
            "ad_slot_id": (row.get("Dimension.AD_UNIT_ID") or "UNKNOWN").strip() or "UNKNOWN",
            "ad_slot_name": ((row.get("Dimension.AD_UNIT_NAME") or row.get("Dimension.AD_UNIT_ID")) or "UNKNOWN").strip() or "UNKNOWN",
...
class AdManagerSoapClient:
    ...
    def fetch_daily_dimension_rows(self, *, task_id: int, report_date: date) -> list[dict[str, object]]:
        return self._fetch_rows_with_definition(
            task_id=task_id, report_date=report_date,
