...
- 现状：生产运行目录 `/srv/adx-account-isolated-collector` 无 Git；独立服务器克隆 `/srv/gitcode/adx-account-isolated-collector` 为干净 `master@86d67e8`，但至少十个运行 Python 文件与克隆不同。业务库为 `/srv/adx-account-isolated-collector/backend/control_plane.db`，约 29 GiB，WAL 模式；MySQL 只含系统库。写前根盘仅约 5 GiB 可用，旧备份约 68 GiB，故新库先转存到本机独立备份目录再加密验证。
...
- 数据保护进度：已停止旧 Web 与 scheduler，确认无数据库占用者，`PRAGMA wal_checkpoint(TRUNCATE)` 返回 `0|0|0`。源库完整性校验、外部副本传输、加密及解密校验正在进行；在这些检查完成前不得删除运行库或旧备份。加密口令仅保存在本机受限文件，不写入 Git 或台账。
- 影响与回滚：停服务期间旧中台不继续拉数或提供 API；完成清理前可从原服务单元恢复。清理后仅可依赖 Git 归档与加密数据库副本恢复；删除内容不可直接撤销。Git 提交号以包含本记录的提交自身为准，发布状态为旧系统退役进行中。
