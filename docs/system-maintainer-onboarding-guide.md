...
- 发布与回滚：实现提交 `7afd21c` 已快进合并并推送至远程 `master`，生产发布门禁进行中，尚未同步前端。首次写前备份因普通用户无权在 `root:root 755` 的备份父目录创建受限目录而在第一步停止；失败路径不存在，未创建或复制数据库、未更新服务器 Git、未同步前端、未重启服务。只读诊断确认 `sudo -n` 可用，正确替代为仅用 sudo 创建精确的 `ubuntu:ubuntu 0700` 备份目录，随后仍由普通用户执行 SQLite online backup 和源库/备份库双 `quick_check`；该替代方案截至本条记录时尚未执行，不得虚报成功。若需撤销，对本次实现提交做反向提交，恢复独立 Action 列和 `colSpan=9`，移除 OAuth 局部样式并同步恢复测试预期；不得删除回归测试掩盖布局变化。失败命令无生产状态变更，无需业务回滚。
