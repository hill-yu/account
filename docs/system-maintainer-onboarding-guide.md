...
#### 2026-08-20 17:14（北京时间）— 当前灰度节点当日小时任务与指标只读查询

- 范围与口径：用户要求查询灰度节点小时任务和当日指标。本轮先通过只读 operator API确认 19 个 `gray=true` 且 schedule enabled 的账户，再严格使用个人 Skill `$adx-report-status-query` v1.0.0、schema profile `production-control-plane-v1`、兼容基线 `9238a8a`，逐账户查询 `beijing_date=2026-08-20`、scope=hourly。北京时间窗口为 UTC `[2026-08-19 16:00, 2026-08-20 16:00)`，跨 Pacific 8月19/20两个源日；查询发生在北京时间约17:14，当天尚未结束，17—23时不得判为缺失或最终完整。
- 任务结论：19个账户的范围内任务均无 pending/in_progress，active_total均为0；每个账户在两个源日范围有8—29个小时任务、其中均存在历史/当日 failed（1—6个），同时均有 succeeded及batch。任务成功与batch存在不代表事实非空或小时完整。
- 事实概览：`uragnv.com`、`zilote.com`、`cpatobe.com`、`ddgjcj.com` 当日账户小时事实均为0行、00—16均无事实；`reboroots.com`与`onlyfungogo.com`覆盖00—14；`coeurdazur.com`仅00—03；`linkzclub.com`覆盖00—10；`stones`覆盖00—09。其他低量或间断节点为 `dddfdc.com`仅03、`ldsjys.com`仅00、`cwpoole.com`为00/08、`skouje.com`为02/05—08/11、`tqchq.com`为02—03、`babyboutiq.com`为00—11/15、`mdqsd.com`为00—04/06—07/09—13、`tlccbc.com`为00—10/15、`sarlvolle.com`为00—02/04—05/15、`theonin.com`为00—11/15—16。空事实只能证明中台当前没有该窗口事实，不能断言Google无数据。
- 指标证据：Requests/Responses/Impressions/Clicks/Revenue分别为：reboroots `274191/182049/120510/7970/11740.719116`；onlyfungogo `570810/343008/277438/17386/2478.259639`；coeurdazur `121102/53257/44240/1742/5368.997733`；linkzclub `118382/68822/51386/3181/2250.171153`；stones `231412/137950/121588/7777/972.828340`；babyboutiq `80900/47734/36111/1919/1810.384700`；tlccbc `38034/24039/9574/678/394.760223`；theonin `19682/7769/3196/158/202.327827`；其余低量节点详见本轮交付表。四个0行节点五项指标均为0。
- 写入与后续：只读查询，未执行 quick_check、补拉、任务创建、schedule/policy修改、服务操作或生产源码变更。当前状态为日内快照；若要补拉，需另行明确授权并先区分已过去小时缺口、未来小时和上游零行，不能根据本次查询自动创建任务。
- Git/发布：仅追加查询记录，不部署运行代码；回滚为反向提交该文档记录，生产无回滚项。

#### 2026-08-21 11:12—11:24（北京时间）— 19 个灰度节点小时任务与跨日修复实证复核

- 目标、授权与口径：用户要求查询全部灰度节点小时任务，并确认 2026-08-20 发布的跨日小时修复是否真正生效。本轮先只读确认 19 个 `gray=true` 且 schedule enabled 的账户，再使用个人 Skill `$adx-report-status-query` v1.0.0、schema profile `production-control-plane-v1`、兼容基线 `9238a8a` 查询 `beijing_date=2026-08-21`；北京时间窗口为 UTC `[2026-08-20 16:00, 2026-08-21 16:00)`。查询时 00—10 为已过去整小时，11 为当前小时，12—23 为未来小时，后两类不得判缺或最终完整。
- 当日任务与事实：19 个账户均无 pending/in_progress，`active_total=0`。任务 succeeded/failed、事实小时、Requests/Responses/Impressions/Clicks/Revenue 如下：`uragnv.com 18/2、无事实、0/0/0/0/0`；`reboroots.com 17/2、00—09、98271/67139/32541/2704/4077.047900`；`zilote.com 18/2、无事实、0/0/0/0/0`；`cpatobe.com 17/2、无事实、0/0/0/0/0`；`ddgjcj.com 17/3、无事实、0/0/0/0/0`；`onlyfungogo.com 14/4、00—10、390318/234072/188778/12421/1579.479609`；`coeurdazur.com 17/2、09—10、44/28/24/0/0.911171`；`linkzclub.com 6/1、00—06、44799/26829/19781/1183/873.337288`；`stones 7/0、00—09、147539/87419/77271/4427/564.094751`；`dddfdc.com 17/2、无事实、0/0/0/0/0`；`ldsjys.com 21/1、06—07、9/0/0/0/0`；`cwpoole.com 21/1、00/02/03、9/0/0/0/0`；`skouje.com 15/3、00/02/03、16/0/0/0/0`；`tqchq.com 17/1、无事实、0/0/0/0/0`；`babyboutiq.com 21/1、00—09、132248/80433/60349/3464/3274.529262`；`mdqsd.com 18/1、00—09、76/56/42/1/2.956463`；`tlccbc.com 21/1、00—09、26945/15567/6827/573/365.721357`；`sarlvolle.com 20/2、07、4/3/1/0/0.000916`；`theonin.com 21/2、00—10、18294/6273/2786/158/231.435700`。其中 6 个账户事实为 0；只能确认中台事实水位，不能推断 Google 无数据。
- 上一完整跨日周期实证：复核北京时间 2026-08-20 对应的 Pacific 2026-08-19 源日后，11 个账户实际产生 2—3 次 `cross_day_finalize`。`uragnv/zilote/ddgjcj` 均为两次成功且水位同为 0；`reboroots` 三次均成功，batch row_count 从 `9415` 收敛为 `9408/9408`；`onlyfungogo/coeurdazur/tqchq/mdqsd` 三次成功且各自连续水位相同；`cpatobe/dddfdc/skouje` 各有一次中间失败，但前后成功批次水位分别稳定在 `4/5/22`。所有上述账户当前无 active，证明“最多三次观察、以成功事实水位判稳、失败不立即误判完成”的代码路径已经在真实周期运行。
- 覆盖缺口与准确结论：生产 `.env` 的 `ADX_COLLECTOR_CROSS_DAY_FINALIZE_ACCOUNT_KEYS` 仅列 12 个账户：`coeurdazur/cpatobe/dddfdc/ddgjcj/linkzclub/onlyfungogo/reboroots/skouje/tqchq/uragnv/zilote/mdqsd`。其中 `linkzclub.com` 为 4 小时 schedule，上一周期没有在 Pacific 01—03 创建窗命中，实际 `cross_day_finalize=0`；另 7 个灰度账户 `stones/ldsjys.com/cwpoole.com/babyboutiq.com/tlccbc.com/sarlvolle.com/theonin.com` 未配置该门禁，实际同样为 0。因此结论是：安全 upsert/防较小快照覆盖的共享 ingestion 修复已全节点部署；跨日观察机制在 11 个命中节点上已实证生效，但尚未覆盖全部 19 个灰度节点，不能宣称跨日问题已全量解决。北京时间 2026-08-21 当日对应的下一轮 Pacific 跨日创建窗要到北京时间约 16:00 后开始，本次 11:24 查询不能用尚未到期的当前周期作结论。
- 失败操作闭环：①第一次批量调用 Skill 错用了 `--account-key/--ssh-host/--ssh-user/--ssh-key`，19 次均在本地 argparse 阶段失败，未建立有效 SSH；正确参数为 `--account-name/--host/--user/--key`。②生产机无 `rg`，代码检索报 command not found；正确替代是先确认工具再用现有 `grep`。③首次猜测 `backend/app/services/fetch_jobs` 路径不存在；改为 `find` 定位实际 `backend/app/collectors`，但广域 find 同时遇到受限备份目录 Permission denied；以后限定运行代码目录。④首次猜错 systemd unit 为 `adx-backend-scheduler`；只读列举后校准为 `adx-control-plane-scheduler`。⑤一次远端 `awk` 因 PowerShell/SSH 转义错误未执行；改为不经 awk 的纯 grep。⑥首次用组合 shell 搜索环境变量因 glob/退出码无结果；最终对已由 unit 确认的 `.env` 执行精确、只读、仅目标键 grep 成功。上述失败均未修改文件、数据库、任务、配置或服务，也未输出秘密；防再犯要求是先读 Skill `--help`、先定位实际目录/unit/工具，再执行固定只读命令，禁止猜参数和路径。
- 写入、影响、回滚、Git/发布：生产全程只读，未 quick_check、未创建或补拉任务、未修改 schedule/policy/.env/源码、未重启服务。唯一写入是本地两份治理文档；回滚为反向提交文档。运行代码仍为已发布的小时安全修复版本，本轮不改变生产发布状态。跨日覆盖扩展或 4 小时 schedule 兼容修复必须另行形成最小方案、测试、独立审阅、备份和用户授权，不能由本次查询自动执行。
- 独立审阅与 Git 状态：独立只读审阅结论为 P0=0、P1=0、P2=0；19 节点计数、时区口径、12 个配置/11 个实际命中、`linkzclub.com` 4 小时 schedule 缺口及“部分解决”结论均通过核对，敏感信息扫描无凭据。当前仅两份治理文档存在本地未提交差异，未 push、未部署；`git diff --check` 通过，仅有仓库既有 LF/CRLF 提示。

#### 2026-08-21 14:33—14:40（北京时间）— 全部 19 个灰度节点跨日保护配置发布

- 目标与授权：用户明确要求把 `linkzclub.com` 改为每小时 schedule，并将此前未完整保护的 8 个灰度节点全部纳入跨日机制；经发现 `stones` 同为 4 小时 schedule 后，用户再次明确授权将其也改为 1 小时。本次目标精确为 `linkzclub.com/stones/ldsjys.com/cwpoole.com/babyboutiq.com/tlccbc.com/sarlvolle.com/theonin.com`，不修改其他账户策略、OAuth、代理或历史数据。
- 独立方案审阅与整改：首轮审阅为 P0=0、P1=3、P2=3。三个 P1 分别是不得静默扩大 `stones` 频率、写入前必须停止 scheduler 防止半新半旧状态、必须避开 Pacific 创建窗并检查当前 attempts/active/exhausted。用户补充授权 `stones`；发布前确认 Pacific 2026-08-20 23:33，不在 `[01,04)` 创建窗，8 个目标当前源日无跨日记录且无 active。P2 通过 key 唯一性、最终集合精确计数、next_run 不超过一小时以及发布后 19/19 映射校验闭环。
- 写前现状与保护点：8 个账户/实例均 active/ready、gray/hourly enabled、OAuth authorized/healthy、无 exclusion；`linkzclub` schedule 28 与 `stones` schedule 29 均为 4 小时 Pacific，其余为 1 小时；`babyboutiq` instance 39 的 `report_account_key` 为 NULL，目标 key `babyboutiq` 无占用。根分区 26%、可用约 113 GB。scheduler 于 14:34:37 停止后创建 `/srv/adx-account-isolated-collector/backups/20260821T063331Z-pre-all-gray-cross-day-protection`，保存 `.env.before` 与 6,818,828,288 字节数据库在线备份，文件 `0600`，源库/备份库双 `quick_check=ok`。
- 实施内容：在单一 `BEGIN IMMEDIATE` 事务中以精确 schedule/account/instance/旧值条件，将 schedule 28（linkzclub）和 29（stones）的 `interval_hours` 从 4 改为 1，`next_run_at` 分别错峰到 UTC 07:18/07:28，均不超过一个小时；将 instance 39/account 39 的 `report_account_key` 从 NULL 改为唯一 `babyboutiq`，三个 rowcount 均为 1。随后以同目录临时文件、原 owner/mode、fsync 和 `os.replace` 原子更新 `.env`：保留原 12 key，新增唯一 `stones-a1/ldsjys/cwpoole/babyboutiq/tlccbc/sarlvolle/theonin`，最终恰好 19 个 key。任一步失败会定向补偿数据库并不启动 scheduler。
- 发布验证：scheduler 于 14:39:12 恢复 active/running，MainPID `1383333`、`NRestarts=0`，控制面 health 正常。读取实际 scheduler 进程环境确认加载 19 个唯一跨日 key；数据库联查 19/19 均唯一映射到 active/ready instance、active policy、gray/hourly enabled、无 exclusion、enabled 1 小时 Pacific schedule。最终生产库 `quick_check=ok`，重启后无新增 failed/blocked/cancelled，warning journal 为空；现存 18 条 active 均为 8 月 1—2 日历史 pending，与本次目标无关且未处理。
- 影响与完成边界：`linkzclub` 和 `stones` 的普通小时任务频率由每 4 小时提高为每 1 小时；其余 6 节点普通频率不变。所有 19 个当前灰度节点现在都具备命中跨日创建门禁的配置条件，但本轮发生在 Pacific 23 时，下一真实创建窗为北京时间约 16:00 后；配置发布已完成，真实 19 节点跨日 attempt、稳定停止或 exhausted 仍须在下一窗口只读观察，不能在窗口前冒充实证完成。
- 回滚：异常时先停止 scheduler，二次备份当前库；从本次保护点只读提取 schedule 28/29 与 instance 39 的写前行，在事务中以当前值为条件定向恢复两 schedule 为 4 小时及其原 next_run/updated_at、恢复 baby key 为 NULL；以 `.env.before` 原 owner/mode 原子恢复环境文件，验证 quick_check 后启动 scheduler。禁止整库恢复。Git/发布状态：仅生产数据/配置写入，无运行代码部署；两份治理文档当前未提交、未 push。
...
#### 2026-09-08 10:42—11:20（北京时间）— 权威维度日报生产发布续作、完整保护点与首轮发布脚本止损

- 目标与授权：用户明确要求继续既定独立维度任务规格并保证不影响其他节点。本轮严格限定为已审阅且已集成 `origin/master@dbb194e` 的 9 个运行文件、`ahzhhj.com`（account 53 / instance 52 / 实际 key `ahzhhj`）及其现有绑定代理；其他账号不得生成维度任务，不修改其 policy、schedule、OAuth、代理或事实数据。
- 续作门禁与写前事实：`daily-dimension-master-integration` 严格 Validate 通过，HEAD 与 `origin/master` 均为 `dbb194e`、工作区原为 clean、治理文件一致。生产 Web/scheduler 初始 active/running、无 collector runtime、磁盘约 91GB 可用；生产 9 文件仍精确匹配旧基线。`ahzhhj.com` 为 active/ready，OAuth authorized/healthy、代理 active、policy gray/hourly/daily/manual 全开启、Pacific 每小时 schedule 正常且无 active 任务；最新成熟 Publisher 业务日 2026-09-07 的核心任务 `75259` succeeded，核心 Requests=3,638，维度任务和维度事实均为 0，其他账号维度任务数为 0。
- 新保护点：停止 scheduler 并确认无 collector runtime 后，新建 `/srv/adx-account-isolated-collector/backups/20260908T025445Z-pre-daily-dimension-isolation`。保护点保存旧 9 文件、`.env`、systemd 文本、服务状态、写前 hash、脱敏 `ahzhhj` 基线和 SQLite online backup；源库与备份库均取得明确 `quick_check=ok`，数据库副本 15,938,109,440 字节，目录/数据库权限受限。昨日 `.incomplete` 文件未复用。
- staging 门禁：从本地 `dbb194e` 精确打包 9 个运行文件，上传后验证归档 SHA-256、成员集合、无非普通文件，并逐文件校验 staging hash；生产 venv 对 staging backend/collector 的 compileall 均通过。生产 Alembic `current` 与 `heads` 均为 `20260802_0014 (head)`，发布前 Web health 正常。
- 首轮发布脚本错误与止损：脚本用通用 `check=True` 调用 `systemctl is-active adx-control-plane-scheduler.service`；scheduler 正常 inactive 时 systemctl 按约定返回退出码 3，却被误判为异常。更严重的是异常处理未以“实际替换是否开始”为条件，虽然新文件尚未替换，仍停止 Web 并把保护点中的 9 个旧文件以相同内容原子重写后再启动 Web。立即停止扩大范围并复核：9 文件 SHA-256 全部仍为写前旧值，Web active/running、health ok，scheduler inactive/dead，`DEPLOYED_FILES` 不存在，数据库和 `.env` 未改，新代码未部署。实际影响仅为一次不必要的 Web 重启、文件 mtime/ inode 可能变化；无其他节点配置或数据变化。
- 其他失败与正确替代：①首次把 `executing-plans` 技能别名少展开一层 `superpowers`，本地只读文件未找到；按实际目录读取。②现行 master 中旧 SOP 文件名不存在，只读定位失败；改为以仓库实际文档和本指南现行流程为准。③只读预检在 `set -euo pipefail` 下把 `pgrep` 零匹配的退出码当失败，后半段未执行；改用显式允许零匹配。④本地发布包命令含删除旧临时包，被安全策略在进程启动前拦截；改用唯一新文件名。⑤长备份远端已完成且生成 `BACKUP_VALID` 后，本机 SSH 客户端未退出；只读确认远端无 Python/备份进程、有效标记存在且唯一匹配本机 SSH PID 后，仅终止本机客户端。以上均未产生额外生产业务写入。
- 修正要求：重跑前必须把 scheduler inactive 视为期望状态，不以退出码 0 作为唯一成功条件；异常处理增加 `replacement_started` 门禁，仅当至少一个目标文件已替换时才允许恢复；发布前再次核旧 hash、Web、scheduler、runtime、保护点有效性。修正脚本需先做独立只读审阅；未通过前不得再次替换。当前状态：**保护点有效、生产旧版健康、scheduler 保持停止、发布尚未完成**。

#### 2026-09-08 11:21—12:10（北京时间）— 权威维度日报独立任务生产发布与 `ahzhhj` 单节点验证

- 目标、授权与范围：继续执行既定书面规格；只发布已集成 `origin/master@dbb194e` 的 9 个固定运行文件，只允许 `ahzhhj.com`（account 53、instance 52、key `ahzhhj`）使用其当前绑定生产代理做首个真实维度日报测试。其他账号不进入维度白名单、不创建维度任务，不修改其 policy、schedule、OAuth、代理或事实；普通 scheduler 仅在全部单节点门禁通过后恢复。
- 部署脚本审阅：针对首轮止损缺陷改为 9 文件旧/新双 manifest、持久 journal、独占锁、信号处理、全量恢复及 Web 一致态启动。独立审阅依次发现并关闭 scheduler 终态、重入返回码和极端 marker `os.replace` 后目录 `fsync` 失败等问题；最终对 532 行、SHA-256 `33876006adc4d7caa70b013e0a8f079b6d966d8bcc9b3f8187bfe3b44b7f196f` 的精确脚本复审为 P0=0、P1=0、P2=0，敏感模式 0。仅该哈希上传至有效保护点并执行。
- 发布结果：部署驱动 PID `3230412` 返回 exit code 0，journal 终态为 `success_ready`，`DEPLOYED_FILES` 存在且 V2 回滚 marker 不存在。生产 9 个文件逐一校验为预期新 SHA-256；backend/collector compileall、Alembic `current=head=20260802_0014`、Web `/health`、无 collector runtime、空自动维度白名单及发布后源库 `quick_check=ok` 全部通过。此时 scheduler 仍为 inactive，维度任务和两类维度事实全库均为 0，证明未提前扩散。
- 真实业务验证：写入前记录 `ahzhhj` 业务日 2026-09-07 核心权威日报基线：Requests=3,638、Responses=3,593、Impressions=3,269、Clicks=221、Revenue=377.719313、eCPM=115.545828、`updated_at=2026-09-07 21:00:22`。通过受保护的 operator API 仅创建 `report_fetch_daily_dimension` 任务 `76122`；任务 succeeded，batch `22763` 为 `admanager_daily_dimension_v1`、131 行。账户维度事实 128 行、Site 维度事实 131 行，两侧汇总 Requests/Responses/Impressions/Clicks 均为 3,638/3,593/3,269/221，Revenue 的维度行求和仅有十进制定点末位舍入差异；两条查询 API 均 HTTP 200、`dimension_data_available=true`。核心日报全部指标与 `updated_at` 保持原值，其他账号维度任务仍为 0。
- 自动化启用与隔离验证：确认生产 `.env` 与保护点 `backend.env.before` 完全一致后，以原子写入只新增唯一一行 `ADX_COLLECTOR_DAILY_DIMENSION_ACCOUNT_KEYS=ahzhhj`，保留 owner/mode 并验证删除该行后的全部字节仍与备份一致。启动 `adx-control-plane-scheduler.service` 后，scheduler/Web 均 active、Web health 正常；短时观察中 `ahzhhj` 仍只有已成功任务 `76122`，其他账号维度任务 0，核心日报未变。普通调度恢复后已有 21 条非维度任务 succeeded、4 条 in_progress，证明生产调度继续运行；另有若干账号小时任务返回通用 `collector_task_failed`，属于各账号采集条件，未据此越权修改。完成全部生产写入后再次执行 SQLite `quick_check` 明确返回 `ok`，同时 scheduler/Web 保持 active。
- 发布后普通任务边界：进一步只读核对发现 `theonin.com`、`babyboutiq.com`、`cgffw.com`、`itmxqm.com`、`ldsjys.com`、`ahzhhj.com`、`uragnv.com`、`reboroots.com`、`onlyfungogo.com` 的本轮最新小时任务 failed；除 `ldsjys.com` 的发布前上一条同样 failed 外，其余 8 个账号发布前上一条均 succeeded。失败日志只有通用 `collector_task_failed`，尚不能确认 OAuth、代理、Google 返回、集中恢复调度负载或其他根因。同一轮已有 21 条普通任务 succeeded，Web/scheduler/SQLite 正常，且新维度路径只为 `ahzhhj` 生成 1 条 succeeded 任务，故没有系统性停摆或维度任务越界证据；但也不把上述 9 个账号声明为正常。本任务按授权仅登记为待单独排查，不补拉、不改其他账号配置，也不以无因果证据的小时失败撤销已验证通过的独立维度发布。
- 本轮新增失败操作与正确替代：①只读预检把保护目录子目录误写为 `runtime-files`，实际为 `runtime`；同时宽泛 `pgrep` 匹配到路径中含 `collector` 的 Web uvicorn，脚本无写入即退出。改为读取实际目录并使用 collector runtime 的精确命令模式。②独立门禁在 `set -o pipefail` 下把无匹配的 `pgrep` 退出码 1 传播给命令替换，导致在已通过九文件/Web 后提前退出；改为读取 `/proc/*/cmdline` 并按精确 runtime 特征计数。③一次 SSH 建连在 10 秒内超时，远端命令未执行；仅提高连接超时后重试同一只读检查。④本地清理临时 `__pycache__` 时把路径校验、删除和 Git 检查合并为单条 PowerShell，安全策略在进程启动前拒绝；随后即使改为唯一绝对文件，删除仍被策略拒绝。未继续绕过策略，该 ignored 缓存不进入 Git，临时源码已由精确补丁删除。以上均未修改生产源码、配置、数据库、服务或节点数据；以后预检必须逐项输出诊断、先校准现场目录，并对允许“零匹配”的命令显式处理返回码，删除操作与检查操作分离且以环境策略为准。
- 影响与回滚：新增能力仅对 `ahzhhj` 自动白名单生效；核心日报任务仍只产核心 batch，维度失败不会阻断核心日报。若出现维度越界、Web/数据库异常或 `ahzhhj` 自动任务异常，先停止 scheduler，将 `.env` 从本保护点原子恢复并确认白名单为空；如需代码回滚，再仅从同一保护点恢复 9 个旧文件，经旧 manifest、compileall、Alembic、Web health 和 SQLite `quick_check` 后恢复普通 scheduler。默认保留任务 `76122`、batch `22763` 和维度事实为审计与有效业务数据，不做整库恢复；若用户另行要求撤销，必须二次备份并按外键顺序定向处理 account 53 / 2026-09-07。
- Git/发布状态：运行代码已发布，功能来源提交 `d71c3c3`、正式基线 `dbb194e`；最终数据库检查通过，首轮两份治理文档独立复审 P0/P1/P2 均为 0，闭环提交 `d9364a8` 已快进推送至远程 `master`。发布后普通任务边界补充的 Git 提交以包含该补充的提交自身为准并继续快进推送；临时部署驱动不纳入 Git。

#### 2026-09-24（北京时间）— 旧生产代码归档与退役前数据库保护

- 目标与授权：用户要求为部署新中台退役旧中台，将当前生产运行代码提交 Git，仅保留新的加密数据库备份并清除旧中台文件和旧备份。清理范围按旧中台专属目录与服务核验，不涉及操作系统、新中台或其他业务目录。
- 现状：生产运行目录 `/srv/adx-account-isolated-collector` 无 Git；独立服务器克隆 `/srv/gitcode/adx-account-isolated-collector` 为干净 `master@86d67e8`，但至少十个运行 Python 文件与克隆不同。业务库为 `/srv/adx-account-isolated-collector/backend/control_plane.db`，31,058,165,760 字节，WAL 模式；MySQL 只含系统库。写前根盘仅约 5 GiB 可用，旧备份约 68 GiB，故使用服务器独立数据盘 `/mnt/datadisk0` 制作压缩加密库。
- 代码归档：从生产运行目录只读打包并提取 backend、collector、frontend、deploy、scripts 的运行源码、测试、迁移和实际前端构建，共 173 个受 Git 跟踪的归档文件；排除数据库、WAL/SHM、`.env`、旧备份、虚拟环境、依赖和缓存。传输包 SHA-256 为 `766C0054130A2731F922DEB5658622B5CB40C67B07D3E2BB593CF078A7749C15`。独立审阅 P0/P1 均为 0；P2 仅为归档原文件自带空白格式，保留原字节。归档是历史证据，不用于自动部署。
- 数据保护完成：停止旧 Web/scheduler、确认无数据库占用者后，`PRAGMA wal_checkpoint(TRUNCATE)` 返回 `0|0|0`；源库 `PRAGMA quick_check` 明确为 `ok`。压缩库 2,077,645,883 字节，`zstd -t` 返回原始长度且通过；用 GPG AES256 加密，解密后压缩字节 SHA-256 与加密前完全一致，均为 `7f981a46ad90e292c331ce951420e61150f8fcb36013a3cf90a2c4937e95cac8`。最终只保留 `/mnt/datadisk0/control_plane-20260924.db.zst.gpg`，大小 2,077,645,985 字节、`root:root 0600`，SHA-256 为 `823334b1aa8408ea28b2b35e492b1a31ce1a866b3fa2688b76f9e9f3074b1e06`；加密口令仅保存在本机受限文件，不写入 Git 或台账。验证后的明文压缩临时文件已删除。未执行第二次解压到 SQLite 文件的全库 `quick_check`；由源库 `ok` 和加密链路逐字节校验支撑备份可恢复结论。
- 清理结果：精确删除旧备份目录、旧运行目录、服务器旧 Git 克隆、旧 systemd 单元、旧 Nginx 站点、旧 `/srv` Node 工具/暂存目录与已识别的 `ubuntu` 家目录旧脚本。`/srv` 当前为空；服务器根盘由约 5 GiB 可用增至约 143 GiB，数据盘仅有上述加密库及文件系统 `lost+found`。Nginx 配置校验通过并已 reload；Nginx/MySQL 仍 active，SSH 与基础系统保留。旧中台不再提供 API 或拉数，新系统尚未部署。
- 异常与防再犯：第一次直接 SCP 原库下载极慢，只形成一个未完成的本机小片段后停止；用户随后要求备份只留服务器，改用独立数据盘就地压缩加密。两次源库 `quick_check` 的 SSH 客户端在远端进程结束后仍悬挂；第一次结果未采信，第二次将结果先写到精确临时文件并取得 `ok`，确认远端已无对应进程后只终止唯一客户端。清理本机未完成数据库片段的两次 `Remove-Item` 命令均被执行策略拒绝，未绕过，需用户在本机手动删除 `D:\adx-production-retirement-20260924\control_plane-20260924.db`；它不是有效备份。后续大库备份优先同机独立挂载盘，验证结果落文件并复读，避免跨网直接下载。
- Git 与回滚：运行代码归档经独立审阅后以 `60d7f30d19789223293208978deda0fbdd56a9c9` 推送到 `origin/codex/production-retirement-20260924`，未合并到 `master`。删除旧项目及历史备份不可直接撤销；恢复需从 Git 归档与上述加密数据库重新部署，且必须保管本机解密口令。状态：生产服务器旧中台退役完成；本机未完成的明文数据库片段仍待用户手动删除；新中台待配置。
