...
| 2026-08-13 | [统一治理文档正式基线与新任务启动门禁](#change-2026-08-13-governance-doc-baseline) | 文档治理 |
...
- 操作内容与原因：检索版本化问题记录、第 22 节、交接 Runbook、主目录未版本化的 2026-08-12 原始台账，以及后续重复交接任务消息记录。区分第一次生产接入与后来未写生产的 SSH 重复尝试，并区分必要门禁与可避免失误。原记录没有逐阶段计时，无法量化端到端耗时、各失误占比或 Google OAuth 页面等待占比；不再作无证据比例判断。
...
- 时间、目标与授权范围：北京时间 2026-08-13，用户确认保留第 22 节原始时间顺序，并新增按问题类型分类索引。本阶段仅形成和提交设计规格，不修改第 22 节历史结构，不连接生产。
- 操作内容与方案：只读统计维护手册章节与现有正式台账记录，形成 `docs/superpowers/specs/2026-08-13-maintenance-guide-category-index-design.md`。设计采用“分类索引 + 显式稳定锚点 + 原始时间线不动”，允许一条记录进入多个分类，索引不复制正文。
- 原因、影响与结果：解决长篇追加式台账只能按时间浏览、难以按 OAuth、调度、小时、日报、部署、节点接入等问题类型检索的问题。当前仅新增规格和本条设计记录，不涉及代码或生产状态。
...
- Git：分支 `codex/production-ops-handoff`；实施提交 `cd7be9e`，本次台账闭环提交紧随其后生成。
- 发布与回滚：不部署生产。回滚只删除本次索引、锚点、本记录和对应问题记录，不移动或覆盖既有时间线正文。

<a id="change-2026-08-13-governance-doc-baseline"></a>

### 2026-08-13 — 统一治理文档正式基线与新任务启动门禁

- 状态：审阅中。
- 需求或问题：不同 worktree 对应不同分支，根目录 `AGENTS.md`、维护手册和问题记录可能处于不同版本；用户要求明确现有 `AGENTS.md` 是否实际生效，并强制每个新窗口、新对话和新任务优先加载该文件。
- 变更内容：在现有仓库根目录 `AGENTS.md` 增加启动门禁、启动状态报告和文档统一版本三条规则；明确 `origin/master` 是三份治理文档的唯一正式事实基线，任务分支内容合并回 `master` 后才成为正式版本。
- 修改原因：避免新任务读取旧 worktree 的规则、遗漏既有问题记录，或把某个任务分支中的文档误认为全项目统一版本。
- 实施方案：新任务先向上定位并完整读取当前工作区实际生效的 `AGENTS.md`，再读取问题记录、维护手册第 1.1 节和第 22 节；随后报告根目录、worktree、分支、HEAD、clean 状态以及相对 `origin/master` 的 ahead/behind。新 worktree 从最新 `origin/master` 创建，既有 worktree显式同步；追加式文档冲突人工保留双方记录，只通过 Git 集成。
- 预期结果与实施后果：新任务在任何修改或生产操作前即可识别实际规则和版本差异；增加一次启动只读核验成本，但减少旧规则、旧 SOP 和跨 worktree 文档漂移造成的误操作。规则无法让已经打开的旧窗口自动重新加载，也无法让既有 worktree 自动同步，仍须在任务启动时主动执行。
