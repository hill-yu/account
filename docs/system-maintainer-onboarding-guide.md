...
### 2026-08-05 — 固化当前生产运行代码基线

- 状态：生产运行代码已导出到隔离分支，待独立审阅、Git 提交与推送。
- 需求或问题：服务器 Git 工作目录、远端 `master` 与实际生产运行目录不是同一代码状态；继续在原本地 `dev` 上开发可能覆盖生产侧有效变更，因此先以实际运行代码建立可追溯基线。
- 变更内容：在本地隔离仓库 `D:\code\adx-mid-platform-production-baseline-20260805` 的 `codex/production-runtime-baseline-20260805` 分支中，以远端 `master` 提交 `81397f2` 为底座，按服务器实际运行目录固化 `collector/app/admanager_soap.py`、`collector/app/adx_report_service.py`、`collector/app/fetcher.py`，并同步纳入服务器上的两个对应测试文件 `collector/tests/test_admanager_soap.py`、`collector/tests/test_fetcher.py`。服务器 Git 工作目录当时为 `master@1317648`，但它不是生产进程直接加载的目录。
- 修改原因：确保后续设计和开发从服务器当前真实执行版本出发，避免把远端仓库中尚未部署的采集逻辑误认为生产基线。
- 实施方案：仅从生产服务器只读导出受版本控制的运行源码；逐文件校验；不复制 `.env`、OAuth、Token、代理、数据库、日志、虚拟环境或构建缓存。生产运行目录的后端源码与远端 `master` 一致；实际执行的 collector 仅上述三个源码文件存在差异。生产前端由 `frontend/dist` 提供，运行目录缺少完整 `frontend/src`，因此未将源码缺失解释为删除，继续保留远端 `master` 的前端源码。
- 预期结果与实施后果：形成可审阅、可提交、可重新克隆的生产代码基线；不会改变服务器文件、进程、任务、配置或数据。远端 `master` 中未实际部署的 collector 变更不会自动进入该生产基线。
- 影响范围：当前仅影响本地隔离分支的五个 collector 文件和本条维护记录；不涉及生产写操作、Google 真实拉取、OAuth、代理、数据库迁移、调度或部署。
- 验证与测试：三个定向测试文件合计 21 项通过；collector 全量测试 87 项通过；`git diff --check` 通过（仅工作区换行提示）；差异凭据模式扫描无命中；五个导出文件与服务器运行目录逐项 SHA-256 校验全部一致。
- 独立审阅：2026-08-05 独立只读审阅通过，无 P0/P1；确认快照边界、五个文件对应关系、凭据安全、测试结论以及保留远端前端源码的处理均可提交。
- Git：本地分支 `codex/production-runtime-baseline-20260805`，提交号待生成；审阅通过前不提交、不推送。
- 发布与回滚：本步骤不部署、不改生产，故无需生产回滚；若基线内容核验失败，丢弃隔离分支并重新只读导出即可，不影响服务器。

### 2026-08-05 — 在生产基线上形成日报与北京时间小时查询设计

- 状态：设计完成，待独立审阅与实施。
- 需求或问题：解决权威日报延迟或回滚、小时任务覆盖日报，以及北京时间凌晨小时数据因 UTC 跨日无法被 user_system 正确查询的问题。
- 变更内容：新增 `docs/superpowers/specs/2026-08-05-authoritative-daily-and-beijing-hourly-design.md`；将 `AGENTS.md` 的指定本地开发目录更新为 `D:\code\adx-mid-platform-production-dev`，开发基线为 `355a24e`。
- 修改原因：确保后续开发建立在已固化的生产实跑版本上，并把用户确认的规则形成可执行、可验收文档。
- 实施方案：日报按账户时区 D+1 的 05:00、06:00、07:00 拉取；每次完整成功均正式覆盖且允许数值下降；失败保留旧版；小时与日报分离；小时 API 按北京时间自然日映射 UTC 区间。
- 预期结果与影响范围：当前仅新增设计和工作目录约束，不修改运行代码、数据库、调度、OAuth、代理或生产服务。
- 验证与测试：已核对设计覆盖用户确认的调度、覆盖、分页完整性、存储、接口兼容、TDD、验收和回滚要求；未执行真实 Google 拉取。
- 独立审阅：2026-08-05 首轮审阅发现完整分页原子替换、跨槽位迟到覆盖和非目标边界 3 个 P1；已按最小方案补充并复审通过，无新增 P0/P1。
- Git：设计与目录规则已提交到本地 `dev`，提交 `e65b759`；维护台账随本条提交。
- 发布与回滚：未发布；如设计需调整，仅修改文档，不影响生产。
### 2026-08-05 — 权威日报刷新与北京时间小时查询阶段性开发

- 状态：开发中，WIP 阶段提交；禁止合并 `master` 或部署生产。
- 需求或问题：落实权威日报 05:00、06:00、07:00 刷新、小时数据与日报隔离，以及 user_system 按北京时间自然日查询小时数据。
- 变更内容：小时入库停止重建权威日报；collector 聚齐核心和维度日报后生成组合批次；backend 增加组合批次投影、轻量版本摘要、日报槽位和北京时间 UTC 查询窗口；新增 Alembic `20260805_0015` 及对应测试。
- 修改原因：避免小时汇总覆盖 Google 权威日报，允许 Google 后续增减值覆盖，并解决北京时间凌晨小时数据因 UTC 跨日无法查询的问题。
- 实施方案：严格依据 `docs/superpowers/specs/2026-08-05-authoritative-daily-and-beijing-hourly-design.md` 按 TDD 分步实施；当前仅提交阶段性本地成果。
- 实施结果与影响范围：backend、collector、数据库迁移和六个小时查询接口存在本地差异；不修改 user_system；未执行真实 Google 拉取、生产配置、数据库或调度变更。
- 验证与测试：阶段检查 backend `149 passed`、collector `89 passed`。已知仍需完成合法零数据替换、任务成功原子化、数据库级槽位唯一性、OAuth recovery 兼容、人工刷新、payload 清理、迁移演练和完整契约测试。
