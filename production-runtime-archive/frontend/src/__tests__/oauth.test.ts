...
describe("parseOAuthCallbackImportJson", () => {
  it("parses a valid callback JSON payload", () => {
    expect(
      parseOAuthCallbackImportJson(
        JSON.stringify({
          state: "state-1",
          ...
          callback_url: "https://jwtnx.com/oauth/google/callback?state=state-1&code=code-1",
          scope: "scope-1",
  ...
  it("rejects JSON without the required callback fields", () => {
    expect(() => parseOAuthCallbackImportJson(JSON.stringify({ state: "state-1" }))).toThrow(
      "Callback JSON is missing one or more required fields.",
    );
  });
});
