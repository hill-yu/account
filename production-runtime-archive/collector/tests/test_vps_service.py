...
def test_vps_fetch_service_enqueue_fetch_reuses_existing_successful_run_for_same_date(tmp_path) -> None:
    from app.vps_models import AdxFetchRun
    from app.vps_service import VpsFetchService

    session_factory = _build_test_session_factory(tmp_path, "enqueue-existing-success.db")
    _seed_active_account(session_factory)
    ...
    with session_factory() as db:
        db.add(
            AdxFetchRun(
                ...
                report_date=date(2026, 5, 14),
                trigger_source="php_manual",
                request_id="req-existing-success",
                status="success",
                row_count=7,
            )
        )
    ...
    with session_factory() as db:
        runs = db.query(AdxFetchRun).order_by(AdxFetchRun.id.asc()).all()
        assert len(runs) == 1
        assert runs[0].request_id == "req-existing-success"
        assert runs[0].status == "success"


def test_vps_fetch_service_enqueue_fetch_retries_existing_failed_run_for_same_date(tmp_path) -> None:
    from app.vps_models import AdxFetchRun
    from app.vps_service import VpsFetchService

    session_factory = _build_test_session_factory(tmp_path, "enqueue-existing-failed.db")
    _seed_active_account(session_factory)
    account_id = _get_account_id(session_factory)
    ...
    with session_factory() as db:
        db.add(
            AdxFetchRun(
                ...
                report_date=date(2026, 5, 14),
                trigger_source="php_manual",
                request_id="req-existing-failed",
                status="failed",
                row_count=0,
    ...
    with session_factory() as db:
        ...
        assert runs[0].request_id == "req-retry"
        assert runs[0].trigger_source == "fetch_schedule"
...
def test_vps_fetch_service_enqueue_fetch_reuses_latest_successful_snapshot_for_same_date(tmp_path) -> None:
    from app.vps_service import VpsFetchService
